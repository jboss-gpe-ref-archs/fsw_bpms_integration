...
The use of the BPM Suite 6 Remote Java Client API is not used in this reference architecture for the following reasons:
...
BPM Suite 6 provides an {executionserver} to interact remotely with the following run-time functionality:

.  BPM Suite 6 Artifact Repository
.  Deployment Unit management
.  Process, Human Task and Rules engines (also known as:  Runtime REST API)

The scope of this reference architecture is limited to interaction with the Process and Human Task engines only.

The _Runtime REST API_ allows for various styles of invocation and content types:

. _Runtime operations_ : conventional REST API that accepts a payload of either _application/xml_ or _application/json_ content-type and whose responses are of type _application/xml_
. _Execute operations_ : generic XML/HTTP API that leverages a BPM Suite 6 Execution Server _CommandObject_ type

On advantage of the _Execute operations_ approach is that it is the only option for sending multiple commands in a single invocation.
The current version of this reference architecture is focused on use of the _Runtime operations_ approach only.
A future version of this reference architecture will also demonstrate use of the _Execute operations_.
...
. click:  _Add from repository_ and followed by _Select_ on the newly uploaded _domain-1.0.jar_ library.
