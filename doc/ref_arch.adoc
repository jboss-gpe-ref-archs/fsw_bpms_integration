...
Correspondingly, these business services should drive the lifecycle of process instances by invoking APIs provided by the BPM tool.
...
Specifically, the BPM Component of FSW allows for starting and signalling of process instances from BPMN2 process definitions that are bundled in the SwitchYard application.
...
The next requirement is to install the domain model library as a static module in both your FSW and BPM Suite 6 servers running in OpenShift.
The intent of deploying the domain model library as a static shared jboss module is to add the domain model libraries to the classpath of BPM Suite 6 Business-Central web application.
In general, there are other approaches to adding libraries to the classpath of web applications that the user of this reference architecture could additionally explore.

Execute the following steps for both your remote BPM Suite 6 and FSW OpenShit gears:
...
The purpose of the _processLifecycleMgmt_ application is to demonstrate a SwitchYard application as a client driving the lifecylce of a remote process instance.
