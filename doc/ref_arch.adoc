...
. _Runtime operations_ : conventional REST API that accepts a payload of whose content-type is either _application/xml_ or _application/json_. Responses back to the HTTP client are of type:  _application/xml_
. _Execute operations_ : XML over HTTP style API that requires a proprietary payload called the: _CommandObject_ .  
